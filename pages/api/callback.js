...
export default async function handler(req, res) {
  ...
  if (!tokenData.access_token) return res.status(400).json({ error: "Invalid token exchange" });
  ...
  res.status(200).send(`<h1>✅ Logged in as ${user.username}#${user.discriminator}</h1>
